...
import { LLMProviderConfig } from '../config/llmProvider.js';

export class OpenRouterContentGenerator implements ContentGenerator {
  private model: ReturnType<typeof createOpenRouter>;
  private llmConfig: Partial<LLMProviderConfig>;
  ...
  private _getCommonParams(request: GenerateContentParameters) {
    ...
    return {
      model: this.model(this.llmConfig.model || 'google/gemini-flash-1.5'),
      messages,
